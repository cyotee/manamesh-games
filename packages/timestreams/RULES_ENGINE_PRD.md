...
1. **Scored-card lifecycle.** Cards do **not** move to score piles card-by-card. Per RULES.md: each scoring slot is processed by scoring the topmost *unscored* card (the Wonky rule); scored/unscored is a per-card flag, and cards physically remain in the era — movable by later effects — until the era completes. Then: (a) delayed effects windowed to "after scoring that era" fire (Pottery), (b) non-slot inventions are discarded, (c) players collect their slot inventions into score piles.
